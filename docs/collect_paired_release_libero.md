...
- `primary`: valid이면서 수정하지 않은 normal rollout이 LIBERO 기준으로 성공한
  주 분석용 pair

LIBERO는 물체를 아직 잡고 있어도 goal predicate가 만족되면 성공할 수 있다.
따라서 normal은 공식 OpenVLA/LIBERO 평가와 동일하게 성공 즉시 종료한다. 자연
release는 primary 필수 조건이 아니며, `t_cmd`·`t_detach`·`t_obs`의 핵심 분석
대상은 강제 조기-release rollout이다.

## 수집 데이터와 필요한 이유

| 수집 데이터 | 필요한 이유 |
|---|---|
| pair/task/seed, initial state 파일·hash, `normal`/`forced_release`, 강제 조건의 `t_cmd`·`t_detach`·`t_obs` | 같은 시작점의 정상 control과 조기 release를 정확히 정렬·비교 |
...
5회 안에 valid 1개와 primary 1개를 만들지 못한 task는 trigger,
target/destination, normal 정책 성공 여부를 점검한다.
